...
verified modules can be integrated into poly-k8s-mcp.
...
Capabilities map to Kubernetes verbs: - `+get+`, `+list+`, `+watch+` →
ReadCapability - `+create+`, `+update+`, `+patch+` → WriteCapability -
`+delete+` → DeleteCapability - `+*+` → AdminCapability
